...
export const ru: Partial<Strings> = {
  ...
  setupNoFrames:
    "Кадров пока нет. Зайди в кастомный матч — в обычном подборе и в ранкеде игра не отдаёт ничего.",

  linkTitle: "Аккаунт",
  linkWhy:
    "Привяжи установку к аккаунту, и она сможет отправлять записанное. Без привязки запись всё равно идёт — но только на эту машину.",
  linkName: "Как назвать эту машину",
  linkBegin: "Привязать установку",
  linkCompare:
    "На странице в браузере показан код. Сверь его с тем, что выше, и подтверди там. Если не совпадают — не подтверждай: просится что-то другое.",
  linkOpenAgain: "Открыть страницу снова",
  linkLinkedAs: (name: string) => `Привязана как ${name}.`,
  linkForget: "Забыть токен на этой машине",
  linkFailed: (reason: string) => `Привязка не завершилась: ${reason}`,
};
